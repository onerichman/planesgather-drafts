-- Clean database migration for player tracking
-- Use this if you get "policy already exists" errors
-- Run this in your Supabase SQL Editor

-- Drop existing policies if they exist (to handle "already exists" errors)
DROP POLICY IF EXISTS "Anyone can view queue participants" ON queue_participants;
DROP POLICY IF EXISTS "Authenticated users can add themselves to queues" ON queue_participants;
DROP POLICY IF EXISTS "Users can update their own queue status" ON queue_participants;
DROP POLICY IF EXISTS "Store owners can update participants in their queues" ON queue_participants;

-- Drop existing table if it exists (to handle "already exists" errors)
...
-- Now create the table and functions cleanly
...
-- Policy to allow access to auth.users for participant queries
CREATE POLICY "Allow access to user emails for participants" ON auth.users
  FOR SELECT USING (true);
...
-- Create participant view function
CREATE OR REPLACE FUNCTION get_queue_participants(queue_id_param INTEGER)
RETURNS SETOF queue_participant_data AS $$
DECLARE
  result queue_participant_data%ROWTYPE;
BEGIN
  FOR result IN
    SELECT 
      qp.user_id,
...
-- Create the composite type for the function
CREATE TYPE queue_participant_data AS (
  user_id UUID,
  email TEXT,
  status TEXT,
...
-- RPC function for backward compatibility
